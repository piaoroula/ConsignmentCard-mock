          ...
          <el-dropdown :hide-on-click="false" trigger="click" style="cursor:pointer">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="scope.row.state!=6">
                <span @click="handleCancelOrder(scope.row, scope.$index)">取消订单</span>
              </el-dropdown-item>
              <el-dropdown-item>
...
export default {
  ...
  data() {
    return {
      ...
      formInfo:{},
      stateList: [{ id: 1, state: '失效'}, { id: 2, state: '处理中'}, {id: 3, state: '成功'}, { id: 4, state: '失败'}, { id: 5, state: '可疑'}, { id: 6, state: '可疑核查'}],
  ...
  methods: {
    ...
    getList() {
      this.searchInfo.beginTime = moment(
        this.searchInfo.times[0]
      ).format('YYYY-MM-DD HH:mm:ss')
      this.searchInfo.endTime = moment(
    ...
    handleCancelOrder(row, index){
      this.$confirm('当前订单将被取消, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        cancelOrder({id: row.id, state: 1}).then(res => {
          if (res.code == 0) {
            this.$message({
              message: '取消订单成功',
              type: 'success'
